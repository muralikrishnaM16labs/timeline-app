...
// ── WALLPAPER PAINTING ──
/*
 * Paints the wallpaper onto BOTH #wallpaper and <html>.
 *
 * The <html> half is the actual fix for the orange band at the bottom: iOS
 * paints the root element's background into the safe areas, and a positioned
 * div can never reach there. Mirroring onto the root makes the safe area show
 * the wallpaper instead of a flat colour.
 *
...
function setWallpaperBackground(value, isImage) {
  ...
  else if (value.includes('gradient')) image = value;
  else                            color = value;

  for (const target of [el, root]) {
